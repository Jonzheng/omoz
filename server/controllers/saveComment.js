...
module.exports = async ctx => {
    ...
    var re_id = body.re_id ? body.re_id : ''
    var re_name = body.re_name ? body.re_name : ''
    var re_content = body.re_content ?  body.re_content : ''
    var id = file_id + new Date().getTime()
    await mysql.raw('insert t_comment (id,record_id,user_id,file_id,master_id,content,re_id,re_name,re_content) values(?,?,?,?,?,?,?,?,?)on duplicate key update status = 1', [id, record_id, user_id, file_id, master_id,content,re_id,re_name,re_content]);
